...
/**
 * Problem : Find the Minimum length Unsorted sub array, sorting which makes the
 * complete array sorted
 * 
 * 
 * Solution: 1) Find the candidate unsorted subarray a) Scan from left to right
 * and find the first element which is greater than the next element. Let s be
 * the index of such an element. In the above example 1, s is 3 (index of 30).
...
public class Problem1Sorting {
	...
	public static void printUnsorted(int arr[], int n) {
		...
		System.out.println(" The unsorted subarray which makes the given array sorted lies between the indees %d and %d"
				+ s + "--" + e);
		return;
	}
